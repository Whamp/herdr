...
mod tests {
    ...
    #[tokio::test]
    async fn ctrl_click_unhandled_url_emits_external_open_host_action() {
        let line = "see https://example.com/pr/307.";
        let col = line.find("example").expect("url host") as u16;
        let (mut app, info) = app_with_screen_bytes(line.as_bytes());

        let actions = app.route_client_events(
    ...
    async fn ctrl_click_url_invokes_plugin_link_handler_but_super_click_does_not() {
        ...
        let ctrl_actions = ctrl_app.route_client_events(
            vec![crate::raw_input::RawInputEvent::Mouse(modified_mouse(
                MouseEventKind::Down(MouseButton::Left),
